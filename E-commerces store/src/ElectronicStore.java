...
public class ElectronicStore {
    ...
    public ArrayList<Product> mostPopItem(){

        ArrayList<Product> mostPop = new ArrayList<Product>();

        for(Product p : stock){
            if(p != null && p.getSoldQuantity() != 0){
                mostPop.add(p);
            }


        }
        Collections.sort(mostPop);

        return mostPop;
    }

    public double updateStockAdd(String product){
        for(Product p:stock){
            if(p!=null && p.toString().equals(product)){
                return p.sellUnits(1);
            }
        }
        return 0.0;
    }

    public double updateStockRemove(String product){
        for(Product p:stock){
            if(p!=null && p.toString().equals(product)){
               return p.sellUnitsRemove(1);
            }
        }
        return 0.0;
    }
